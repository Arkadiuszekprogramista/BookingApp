...
func TestForm_Valid(t *testing.T) {
	r := httptest.NewRequest("POST","/testFormIsValid", nil)
	form := New(r.PostForm)

	isValid := form.Valid()
	if !isValid {
		t.Error("got invalid when shuld have been valid")
	}
}

func TestForm_Required(t *testing.T) {
	r := httptest.NewRequest("POST","/testFormRequired", nil)
	form := New(r.PostForm)

	form.Required("a","b","c")
	if form.Valid() {
		t.Error("form shows valid when required filds are missing")
	}
	...
	r, _ = http.NewRequest("POST","/testFormRequired", nil)
...
func TestForm_Has(t *testing.T) {
	r := httptest.NewRequest("POST","/testFormHas", nil)
	form := New(r.PostForm)
	
	has := form.Has("someField")
	if has {
	...
	has = form.Has("a")
	if !has {
		t.Error("show form does not have filed when it shoud")
	}

}

func TestForm_MinLength(t *testing.T) {
	r := httptest.NewRequest("POST","/testFormMinLength", nil)
	form := New(r.PostForm)
	...
	if isError == "" {
		t.Error("shoud have an errpr, but did not get one")
	}
	...
	if isError != "" {
		t.Error("shoud not have an error, but got one")
	}
